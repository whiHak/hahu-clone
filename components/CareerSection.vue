...
<template>
  <div class="container mx-auto px-4 py-8">
    <section class="flex flex-col items-center">
      ...
      <div class="grid grid-cols-1  sm:grid-cols-4 md:grid-cols-5 jusic lg:grid-cols-6 gap-6 w-full  mt-4">
        <div v-for="(sector, idx) in sectors" :key="idx" class="bg-white rounded-xl shadow-md p-4 flex flex-col items-center justify-center text-center hover:shadow-lg transition">
          <img :src="sector.icon" :alt="sector.name" class="w-16 h-16 mb-4" />
          <h2 class="text-lg font-bold text-[#009688] mb-2">{{ sector.name }}</h2>
          <p class="text-gray-600 text-base">{{ sector.positions }} Open positions</p>
        </div>
      </div>
    </section>
  </div>
